...
2. No FAIL lines in output
  ...
  <done>
All local API integration tests pass: health, project info, auth, CORS, and conversations endpoints verified.
  </done>
</task>

<task type="auto">
  <name>Task 3: Full round-trip integration test (edit-verify-revert via admin chat)</name>
  <files></files>
  <action>
This is a MANUAL round-trip test that must be performed in a browser. Start the Vite dev server and test the complete admin pipeline.
...
Send: "Please revert the last commit with `git revert HEAD --no-edit`"

Verify:
- Claude runs the revert
...
- Open a second panel (if multi-panel is supported) — verify both render
...
NOTE: Since this is an automated task, use curl to simulate the chat API call for initial verification, then document the browser steps for the human verification checkpoint in Plan 03. The critical automated verification is:
```bash
# Simulate chat API call to verify backend processes messages
PASS=$(grep CULTIVATE_ADMIN_PASSWORD ~/Code/agent-backend/.env | cut -d= -f2)
curl -s -N \
  -H "Authorization: Bearer $PASS" \
  -H "Content-Type: application/json" \
  -d '{"message":"What is the project name in site.ts?","conversationId":"test-integration"}' \
  http://localhost:3100/api/v1/projects/cultivate-wellness/chat | head -20
```
Verify a streamed SSE response is returned (lines starting with `data:`).
  </action>
  <verify>
1. Chat API returns streamed SSE data when sent a message via curl
2. Dev server at localhost:5173 loads without errors
3. `npm run build` still succeeds after any test artifacts are cleaned up
  </verify>
  <done>
Agent-backend processes chat messages and returns streamed responses for the cultivate-wellness project. The admin chat pipeline (frontend -> backend -> Claude) is verified working locally. Round-trip edit-verify-revert test proves file tools and git integration work.
  </done>
</task>
...
2. All API integration tests pass: `bash scripts/test-local-admin.sh` outputs "ALL CHECKS PASSED"
3. Chat streaming works: curl to chat endpoint returns SSE data
4. Cloudflare tunnel is live: `curl -s -o /dev/null -w "%{http_code}" https://cultivate-agent.drzach.ai/health` returns 200
</verification>
